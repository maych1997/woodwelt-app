...
const CategoryStack = observer(({route, navigation}) => {
  ...
  if (!category) {
    ...
    if (load_featured || load_latest || load_sale || load_new) {
      return <View style={{flex:1,justifyContent:'center',alignItems:'center'}}><ActivityIndicator  color={'#FA6E49'} animating={load_featured || load_latest || load_sale || load_new?true:false}></ActivityIndicator></View>
    }
    ...
    if (loading)
      return (
        <View style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
          <ActivityIndicator  color={'#FA6E49'} animating={loading}></ActivityIndicator>
        </View>
      );
